...
export default function WorkoutScreen() {
  ...
  // Prevents re-running the restore after the first successful cache load.
  const cacheRestoredRef = useRef(false);
  // Guards the debounced save from writing empty initial state before restore runs.
  const saveEnabledRef = useRef(false);

  useEffect(() => {
    if (!sheet || cacheRestoredRef.current) return;

    void (async () => {
      const saved = await loadSessionState(sessionId);
  ...
  useEffect(() => {
    // Skip if cache was restored — cached notes take priority over server notes
    // on this mount (the user's in-progress edits aren't persisted to the server).
    if (!exerciseNotes || cacheRestoredRef.current) return;
